...
public class BoardView1 extends JPanel implements IBoardView, Scrollable,
    ...
    */
    ...
    // used to mark hexes for debug purposes
    Set<Coords> markedHexes = new HashSet<>();
    
    // Ideally taken from the hex terrain but doesn't work:
    private final int woodsHeight = 2;
    ...
    private Image createBlurredShadow(Image orig) {
        ...
        // is the image present and loaded?
        if ((orig == null) ||
        ...
        // return the stored shadow if present
        int hash = orig.hashCode();
        Image storedShadow = shadowcache.get(hash);
        if (storedShadow != null) {
            return storedShadow;
        }
        
        // a slightly bigger image to give room for blurring
        BufferedImage shadow = gConfig.createCompatibleImage(
                (int)((orig.getWidth(this)+10)*scale), 
                (int)((orig.getHeight(this)+10)*scale),
                Transparency.TRANSLUCENT);
        
        Graphics gm = shadow.getGraphics();
        gm.drawImage(orig,
                (int)(5*scale),
                (int)(5*scale),
                (int)(orig.getWidth(this)*scale), 
                (int)(orig.getHeight(this)*scale),null);
        gm.dispose();
        
        shadow = createShadowMask(shadow);
        shadow = blurOp.filter(shadow, null);
        if (game.getPlanetaryConditions().getLight() != PlanetaryConditions.L_DAY) {
            shadow = blurOp.filter(shadow, null);
        }
        
        shadowcache.put(hash, shadow);
        return shadow;
    ...
    private Image createBlurredShadowNoScale(Image orig) {
        ...
        // a slightly bigger image to give room for blurring
        BufferedImage shadow = gConfig.createCompatibleImage(
                (int)((orig.getWidth(this)+10)/shadowscale), 
                (int)((orig.getHeight(this)+10)/shadowscale),
                Transparency.TRANSLUCENT);
        ...
        gm.drawImage(orig,
                (int)(5/shadowscale),
                (int)(5/shadowscale),
                (int)(orig.getWidth(null)/shadowscale), 
                (int)(orig.getHeight(null)/shadowscale),null);
        gm.dispose();
    ...
    /** Clears up saved shadow images. For zooming or board changes. */ 
...
//        terrainShadows.clear();
    ...
    /** Used to add hexes to the shadow lists. No outside utility. */
    private void addToShadowList(HashMap<Coords, Set<Coords>> map, 
            Coords shadowed, Coords shadower) {
        if ((!game.getBoard().contains(shadower)) ||
                (!game.getBoard().contains(shadowed))) {
            return;
        }
        if (!map.containsKey(shadowed)) {
            map.put(shadowed, new HashSet<Coords>());
        }
        map.get(shadowed).add(shadower);
    }
    
    /** Used to find a hex from a board pixel position. Does not 
     * consider Iso mode nor the board screen padding. Only used for shadows. */
    private Coords getCoordsFromBoardPoint(Point p) {
        // simple coordinate calc
        ...
        // check if the hex at cc actually contains the point
        if (!HexDrawUtilities.getHexFull(
                getBoardPointFromCoords(cc),scale).contains(p)) {
            // ...it does not:
            // find the adjacent hex that contains p
            for (int dir: allDirections) {
                Coords cn = cc.translated(dir);
                if (HexDrawUtilities.getHexFull(
                        getBoardPointFromCoords(cn),scale).contains(p)) {
                    cc = cn;
                    break;
                }
            }
    ...
    /** Used to find a board pixel position for a hex. Does not 
     * consider Iso mode nor the board screen padding. Only used for shadows. */
    private Point getBoardPointFromCoords(Coords c) {
        // X: 3/4 the hex width times x
        int x = c.getX() * (int) (HEX_WC * scale);
        
        // Y: hex height times y
        // and adjust for the displaced odd columns
        int y = (c.getY() * (int) (HEX_H * scale));
        if (c.isXOdd()) y+= (int) ((HEX_H / 2) * scale);
        
        return new Point(x, y);
    }
    
    /** Adds an easily visible red mark to the hex at Coords c. */
    public void markHex(Coords c) {
        markedHexes.add(c);
    }
    
    /** Clears all marks added via markHex(). */
    public void clearMarks() {
        markedHexes.clear();
    }
    
    
    private void findShadowedHexes(Coords startPoint, HashMap<Coords,Set<Coords>> hexMap, int height, int lvlDiff, int[] dirs) {
        //    int lvlDiff = level - board.getMinElevation();
        IBoard board = game.getBoard();
        IHex hex = board.getHex(startPoint);
        int level = hex.getLevel();

        // Get the shadow line end point
        Point s1 = getBoardPointFromCoords(startPoint);
        // Start from the center of the hex
        s1.x += ((HEX_W/2)*scale);
        s1.y += ((HEX_H/2)*scale);
        int nx = s1.x + (int)(lightDirection[0]*lvlDiff*scale);
        int ny = s1.y + (int)(lightDirection[1]*lvlDiff*scale);
        Coords endPoint = getCoordsFromBoardPoint(new Point(nx, ny));

        // Add hexes along the shadow line as shadowed hexes
        for (Coords curP: Coords.intervening(startPoint, endPoint)) {

            // add the three adjacent hexes according to light direction
            for (int i = 0; i<3; i++) {
                Coords cd = curP.translated(dirs[i]);
                // only add as a shadowed hex if ...
                // on the board and lower in level
                if (board.contains(cd) 
                        && level+height > board.getHex(cd).getLevel()) {

                    addToShadowList(hexMap, cd, startPoint);
                }

                // TODO: if all 3 hexes are at least the same level, break off the search
                // this will supersede the selection above (for terrain shadows), if the adjacent 3 are same level already, no shadow.
            }
        }
    }
    ...
    // This sets the size of the fixed length shadow base images in units of 
    // HEX_W*scale. This should be as small as possible to save memory
    // while still large enough not to create artifacts.
    float fixedShadowBaseSize = 4;
    
    /** Prepares a base shadow image. Only used for shadows. */
    private BufferedImage prepareShadow(Image singleShadow, Point shadowEnd) {

        // create the blurred and scaled hex base shadow
        Image baseShadow = createBlurredShadow(singleShadow);

        int imageSize = (int)(fixedShadowBaseSize*hex_size.width);

        BufferedImage finishedShadow = gConfig.createCompatibleImage(
                imageSize, imageSize,
                Transparency.TRANSLUCENT);

        Graphics g = finishedShadow.getGraphics();

        // start and end point of the shadow are points on a circle
        // of radius 1.5*hex_size.width around the middle of the image
        // their connecting line is in the direction of the shadows
        Point2D p1 = new Point2D.Double(
                imageSize/2 - fixedShadowBaseSize/2*hex_size.width*lightDirection[0]/Math.abs(shadowLength),
                imageSize/2 - fixedShadowBaseSize/2*hex_size.width*lightDirection[1]/Math.abs(shadowLength));

        // quality of the shadows, higher q = worse
        int q = 6;
        double deltaX = lightDirection[0]/Math.abs(shadowLength)*scale*q;
        double deltaY = lightDirection[1]/Math.abs(shadowLength)*scale*q;

        // repeat the image along the shadowline to create a single
        // elongated shadow
        for (int i = 0; i<3*HEX_W/q; i++) {
            g.drawImage(baseShadow, (int)p1.getX(), (int)p1.getY(), null);
            p1.setLocation(p1.getX()+deltaX, p1.getY()+deltaY);
        }
        
        g.dispose();
        shadowEnd.x = (int)p1.getX();
        shadowEnd.y = (int)p1.getY();
        
        imagesum++;
        memsum += imageSize*imageSize*4/1000;

        return finishedShadow;
    }

    
    /** For each Hex, a set of hexes that will cast a shadow on it. */
    HashMap<Coords,Set<Coords>> terrainShadowHexes = new HashMap<>();
    /** The terrain hex shadow base image, about 3 hexes long. */
    BufferedImage terrainShadow;
    /** A reference point in the hex shadow base image used for correct placement. */ 
    Point terrainShadowEnd = new Point();
    ...
    Map<Integer,BufferedImage> buildingShadows = new HashMap<>();
    HashMap<Coords,Set<Coords>> buildingShadowHexes = new HashMap<>();
    
    ImageCache<Integer, Image> shadowcache = new ImageCache<>(); 
    final int maxDiff = 150; // limit all diffs to this value
    // 
    int maxLevelDiff;
    
    /** Prepares shadow information and images for hills, trees and buildings. */
    public void prepareShadows() {
        ...
        if (!GUIPreferences.getInstance().getShadowMap() ||
                ...
                (!isTileImagesLoaded()) ||
                // map editor:
                (game.getPhase() == IGame.Phase.PHASE_UNKNOWN)) {
            return;
        }
        ...
        */
        ...
        // Get the shadow direction vector
        ...
        if (Math.abs(shadowLength) < 0.1) {
            lightDirection[0] = 0.01;
            lightDirection[1] = 0.01;
        }
        
        // Get the hex direction that is closest to the shadow direction
        // and the two adjacent directions
        double absShadowDir = shadowDir;
        if (shadowLength < 0) absShadowDir += 180;
        absShadowDir = absShadowDir % 360;
        int mDir = ((480-(int)absShadowDir) / 60) % 6;
        int[] sDirs = { (mDir+1)%6, (mDir+5)%6, mDir%6 };
        
        // Prepare an elevation shadow image; this is the hex image
        // repeated along a shadow line. A single shadow image will
        // be prepared that is about 3 hexes long. This is enough
        // as the shadow is drawn in drawHex() for single hexes, so
        // the image only needs to be big enough to cover one hex.
        Image hexShadow = tileManager.getHexMask();
        terrainShadow = prepareShadow(hexShadow, terrainShadowEnd);
        
        // now, for all hexes, check if they shadow any other hexes
        for (Coords c: allBoardHexes()) {
            ...
            // a hex will cast shadows only if it is not adjacent
            // to same height hexes in the three shadow directions
            boolean castsShadow = false;
            for (int dir: sDirs) {
                Coords cd = c.translated(dir);
                if (board.contains(cd) && 
                        (board.getHex(cd).getLevel() < level)) { 
                    castsShadow = true;
                }
            }
            if (castsShadow) { 
                // get the maximum possible length of shadow
                int lvlDiff = level - board.getMinElevation();
                findShadowedHexes(c, terrainShadowHexes, 0, lvlDiff, sDirs);
            }

            
            // ### 2 Woods/Jungle Shadows ##########
            ...
            if (hex.containsTerrain(Terrains.WOODS) ||
                ...
                // Find the list of shadowed hexes for this woods

                // Woods shadow the hex itself
                addToShadowList(woodsShadowHexes, c, c);

                // TODO: get the woods height from Terrain?
                // getting the terrain level returns 1 (wrong)
                // height = hex.terrainLevel(Terrains.WOODS);
                
                height = woodsHeight;
                int lvl = height + level;
                int lvlDiff = lvl - board.getMinElevation();
                findShadowedHexes(c, woodsShadowHexes, height, lvlDiff, sDirs);

                // prepare the image for this woods unless it's already there
                List<Image> supers = tileManager.supersFor(hex);

                if (!supers.isEmpty()) {
                    int hash = supers.get(supers.size()-1).hashCode();
                    if (!woodsShadows.containsKey(hash)) {
                        Image woods = createBlurredShadow(supers.get(supers.size()-1));
                        if (woods == null) {
                            return;
                        }

                        Dimension eSize = new Dimension(
                                (int)((Math.abs(lightDirection[0])*height+HEX_W+10)*2*scale), 
                                (int)((Math.abs(lightDirection[1])*height+HEX_H+10)*2*scale));

                        BufferedImage elevShadow = gConfig.createCompatibleImage(
                                eSize.width, 
                                eSize.height,
                                Transparency.TRANSLUCENT);
                        ...
                        Point2D p1 = new Point2D.Double(
                                (Math.abs(lightDirection[0])*height+HEX_W)*scale, 
                                (Math.abs(lightDirection[1])*height+HEX_H)*scale);
                        for (int i = 0; i<Math.abs(shadowLength)*height/q; i++) {
                            gS.drawImage(woods, (int)p1.getX(), (int)p1.getY(), null);
                            p1.setLocation(p1.getX()+deltaX, p1.getY()+deltaY);
                        }
                        gS.dispose();
...
//            IHex hex = board.getHex(c);
            if (hex.containsTerrain(Terrains.BUILDING)) {
                height = hex.terrainLevel(Terrains.BLDG_ELEV);
                
                // Find the list of shadowed hexes for this woods
                
                // Woods shadow the hex itself
                addToShadowList(buildingShadowHexes, c, c);
                
                // get the maximum possible length of shadow
                int lvl = height + board.getHex(c).getLevel();
                int lvlDiff = lvl - board.getMinElevation();
                findShadowedHexes(c, buildingShadowHexes, height, lvlDiff, sDirs);
                /*
                // Get the shadow line end point
                Point s1 = getBoardPointFromCoords(c);
                // Start from the center of the hex
                s1.x += hex_size.width/2; 
                s1.y += hex_size.height/2;
                int nx = s1.x + (int)(lightDirection[0]*lvlDiff*scale);
                int ny = s1.y + (int)(lightDirection[1]*lvlDiff*scale);
                Coords endPoint = getCoordsFromBoardPoint(new Point(nx, ny));
                ...
                for (Coords curP: Coords.intervening(c, endPoint)) {
                    ...
                    // add the three adjacent hexes according to light direction
                    for (int i = 0; i<3; i++) {
                        Coords cd = curP.translated(sDirs[i]);
                        // only add as a shadowed hex if ...
                        // on the board and lower in level
                        if (board.contains(cd) 
                                && lvl > board.getHex(cd).getLevel()) {

                            addToShadowList(buildingShadowHexes, cd, c);
                        }
                    }
                }*/
/*
                ...
                s1.x += ((HEX_W/2)*scale);
                s1.y += ((HEX_H/2)*scale);
                int nx = s1.x + (int)(lightDirection[0]*height*scale);
                int ny = s1.y + (int)(lightDirection[1]*height*scale);
                Coords endPoint = getCoordsFromBoardPoint(new Point(nx, ny));

                // Add hexes along the shadow line as shadowed hexes
                for (Coords curP: Coords.intervening(c, endPoint)) {
                    addToShadowList(buildingShadowHexes, curP, c);
                    addToShadowList(buildingShadowHexes, curP.translated(sDirs[0]), c);
                    addToShadowList(buildingShadowHexes, curP.translated(sDirs[1]), c);
                    addToShadowList(buildingShadowHexes, curP.translated(sDirs[2]), c);
                }
                */
                // prepare the image for this woods unless it's already there
                List<Image> supers = tileManager.supersFor(hex);
/*
                if (!supers.isEmpty()) {
                    int hash = supers.get(supers.size()-1).hashCode() + height;
                    if (!buildingShadows.containsKey(hash)) {
                        Image bldg = createBlurredShadow(supers.get(supers.size()-1));
                        if (bldg == null) {
                            return;
                        }
//                        double lDiff = height;
                        ...
                        Point2D p1 = new Point2D.Double(
                                ...
                                (Math.abs(lightDirection[1])*height+HEX_H)*scale);
                        for (int i = 0; i<Math.abs(shadowLength)*height/q; i++) {
                            gS.drawImage(bldg, (int)p1.getX(), (int)p1.getY(), null);
                            p1.setLocation(p1.getX()+deltaX, p1.getY()+deltaY);
                        }
                        gS.dispose();
                ...
                q = 3;
                deltaX = lightDirection[0]/Math.abs(shadowLength)*scale*q/shadowscale;
                deltaY = lightDirection[1]/Math.abs(shadowLength)*scale*q/shadowscale;
                
                if (!supers.isEmpty()) {
                    int hash = supers.get(supers.size()-1).hashCode() + height; // cmerzy: 144 w/o height, 238 w/ height, SHAD_BLD: 319-183
                    if (!buildingShadows.containsKey(hash)) {
                        Image bldg = createBlurredShadowNoScale(supers.get(supers.size()-1));
                        if (bldg == null) {
                            return;
                        }
//                        double lDiff = height;
                        Dimension eSize = new Dimension(
                                (int)((Math.abs(lightDirection[0])*height+HEX_W+10)*2/shadowscale), 
                                (int)((Math.abs(lightDirection[1])*height+HEX_H+10)*2/shadowscale));
                        ...
                        Graphics gS = elevShadow.getGraphics();
                        Point2D p1 = new Point2D.Double(
                                (Math.abs(lightDirection[0])*height+HEX_W)/shadowscale, 
                                (Math.abs(lightDirection[1])*height+HEX_H)/shadowscale);
                        for (int i = 0; i<Math.abs(shadowLength)*height/q; i++) {
                            gS.drawImage(bldg, (int)p1.getX(), (int)p1.getY(), null);
                            p1.setLocation(p1.getX()+deltaX, p1.getY()+deltaY);
                        }
...
/*
                // Prepare the shadow image for this building tile
                List<Image> supers = tileManager.supersFor(hex);

                if (!supers.isEmpty()) {
                    // The building should be the last super image as it's the topmost
                    int hash = supers.get(supers.size()-1).hashCode();
//                    hash += height;

                    if (!buildingShadows.containsKey(hash)) {
//                        Image bld = createBlurredShadow(supers.get(supers.size()-1));
                        Point bldEnd = new Point();
                        buildingShadows.put(hash, prepareShadow(supers.get(supers.size()-1), bldEnd));
                        imagesum++;
                        /*
                        ...
                        if (bld != null) {
                            ...
                            for (int i = 0; i<n*height; i++) {
                                gS.drawImage(bld, (int)p1.getX(), (int)p1.getY(), null);
                                p1.setLocation(p1.getX()+deltaX, p1.getY()+deltaY);
                            }
                            gS.dispose();
                            buildingShadows.put(hash, elevShadow);
...
//        }
        ...
        for (Coords c: allBoardHexes()) {
            ...
            if (hex.containsTerrain(Terrains.BRIDGE)) {
                ...
                // the woods will always shadow the hex itself (its own floor)
                addToShadowList(buildingShadowHexes, c, c);
                addToShadowList(buildingShadowHexes, c.translated(sDirs[0]), c);
                addToShadowList(buildingShadowHexes, c.translated(sDirs[1]), c);
                addToShadowList(buildingShadowHexes, c.translated(sDirs[2]), c);
                addToShadowList(buildingShadowHexes, c.translated(sDirs[0]).translated(mDir), c);
                addToShadowList(buildingShadowHexes, c.translated(sDirs[1]).translated(mDir), c);
                addToShadowList(buildingShadowHexes, c.translated(sDirs[2]).translated(mDir), c);
                // TODO: enough?

                // check which other hexes are shadowed
                // and add the current hex to the list shadowing those hexes
                List<Image> orthos = tileManager.orthoFor(hex);

                if (!orthos.isEmpty()) {
                    int hash = orthos.get(orthos.size()-1).hashCode();
                    int h = hex.terrainLevel(Terrains.BRIDGE_ELEV);
                    hash += h;
                    
                    if (!buildingShadows.containsKey(hash)) {
                        Image bld = createBlurredShadow(orthos.get(orthos.size()-1));
                        if (bld != null) {
                            Dimension eSize = new Dimension(
                                    (int)((Math.abs(lightDirection[0])*h+HEX_W+10)*2*scale), 
                                    (int)((Math.abs(lightDirection[1])*h+HEX_H+10)*2*scale));
                            ...
                            Graphics gS = elevShadow.getGraphics();
                            Point2D p1 = new Point2D.Double(
                                    (Math.abs(lightDirection[0])*h+HEX_W)*scale, 
                                    (Math.abs(lightDirection[1])*h+HEX_H)*scale);
                            p1.setLocation(p1.getX()+deltaX*q*h, 
                                    p1.getY()+deltaY*q*h);
                            for (int i = 0; i<q; i++) {
                                gS.drawImage(bld, (int)p1.getX(), (int)p1.getY(), null);
                            }
        ...
        System.out.println("Approximate memory use of terrain shadow base images: "+memsum+" kB");
        System.out.println("Number of terrain shadow base images: "+imagesum);

        long tT5 = System.nanoTime()-stT;
        System.out.println("Time to prepare the shadows: "+tT5/1e6+" ms");
    ...
    private void drawHex(Coords c, Graphics boardGraph,
        ...
        if (guip.getBoolean(GUIPreferences.SHADOWMAP) &&
            ...
            BufferedImage tempS = gConfig.createCompatibleImage(
                    ...
                    Transparency.TRANSLUCENT);
            Graphics gs = tempS.getGraphics();

            IBoard board = game.getBoard();
            Point pC = getHexLocationNoIso(c); 
            
            if ((terrainShadow != null) &&
                    terrainShadowHexes.containsKey(c)) {
                // Cycle the hexes that shadow the current one
                for (Coords shadowing: terrainShadowHexes.get(c)) {
                    // get the level difference and position of this shadowing hex
                    IHex shadowingHex = board.getHex(shadowing);
                    int levelDiff = shadowingHex.getLevel() - level;
                    Point pS = getHexLocationNoIso(shadowing);

                    // Relative position of shadowing hex and this hex
                    Point hexVector = new Point(
                            pS.x-pC.x, 
                            pS.y-pC.y );
                    
                    // check if the shadow is too long, if so shorten it
                    // so that the shadow base image overlaps the current hex
                    double distance = hexVector.distance(0, 0);
                    distance += fixedShadowBaseSize/2*hex_size.width;
                    if (scale*Math.abs(shadowLength)*levelDiff < distance) distance = scale*Math.abs(shadowLength)*levelDiff;
                    
                    Point shadowEnd = new Point(
                            hexVector.x+(int)(lightDirection[0]/Math.abs(shadowLength)*distance), 
                            hexVector.y+(int)(lightDirection[1]/Math.abs(shadowLength)*distance) );

                    // draw the correctly displaced shadow base image
                    gs.drawImage(terrainShadow, 
                            shadowEnd.x-terrainShadowEnd.x-(int)(5*scale), 
                            shadowEnd.y-terrainShadowEnd.y-(int)(5*scale), null);
                }
            }

            if (woodsShadowHexes.containsKey(c)) {
                for (Coords shadowing: woodsShadowHexes.get(c)) {
                    IHex shadowingHex = board.getHex(shadowing);
                    List<Image> osupers = tileManager.supersFor(shadowingHex);
                    if (!osupers.isEmpty()) {
                        int hash = osupers.get(osupers.size()-1).hashCode();
                        double shadowinglevel = shadowingHex.getLevel();
                        double xCorr = -(shadowinglevel-level)*lightDirection[0]*scale;
                        double yCorr = -(shadowinglevel-level)*lightDirection[1]*scale;
                        BufferedImage or = woodsShadows.get(hash);
                        if (or != null) {
                            Point pS = getHexLocationNoIso(shadowing);
                            gs.drawImage(or, 
                                    pS.x-pC.x-(int)(xCorr+(Math.abs(lightDirection[0])*woodsHeight+HEX_W+5)*scale), 
                                    pS.y-pC.y-(int)(yCorr+(Math.abs(lightDirection[1])*woodsHeight+HEX_H+5)*scale), null);
                        }
                    }
                }
            }
            ...
            if (buildingShadowHexes.containsKey(c)) {
                for (Coords shadowing: buildingShadowHexes.get(c)) {
                    IHex shadowingHex = board.getHex(shadowing);
                    int bldgHeight;
                    List<Image> osupers;
                    if (shadowingHex.containsTerrain(Terrains.BUILDING)) {
                        osupers = tileManager.supersFor(shadowingHex);
                        bldgHeight = shadowingHex.terrainLevel(Terrains.BLDG_ELEV);
                    } else {
                        osupers = tileManager.orthoFor(shadowingHex);
                        bldgHeight = shadowingHex.terrainLevel(Terrains.BRIDGE_ELEV);
                    }
                    if (!osupers.isEmpty()) {
                        int hash = osupers.get(osupers.size()-1).hashCode() + bldgHeight;
                        double shadowinglevel = shadowingHex.getLevel();
                        double xCorr = -(shadowinglevel-level)*lightDirection[0]*scale/shadowscale;
                        double yCorr = -(shadowinglevel-level)*lightDirection[1]*scale/shadowscale;
                        BufferedImage or = buildingShadows.get(hash);
                        if (or != null) {
                            Point pS = getHexLocationNoIso(shadowing);
//                            gs.drawImage(or, 
//                                    pS.x-pC.x-(int)(xCorr+(Math.abs(lightDirection[0])*bldgHeight+HEX_W+5)*scale), 
//                                    pS.y-pC.y-(int)(yCorr+(Math.abs(lightDirection[1])*bldgHeight+HEX_H+5)*scale), null);
                            
                            ((Graphics2D)gs).setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
                            
                            gs.drawImage(or, 
                                    pS.x-pC.x-(int)(xCorr+(Math.abs(lightDirection[0])*bldgHeight+HEX_W+5)*scale), 
                                    pS.y-pC.y-(int)(yCorr+(Math.abs(lightDirection[1])*bldgHeight+HEX_H+5)*scale), 
                                    (int)(or.getWidth()*scale*shadowscale), (int)(or.getHeight()*scale*shadowscale),
                                    null);
                        }
                    }
                }
            ...
            if (buildingShadowHexes.containsKey(c)) {
                for (Coords shadowing: buildingShadowHexes.get(c)) {
                    IHex sHex = board.getHex(shadowing);
                    List<Image> osupers;
                    int bldgHeight;
                    if (sHex.containsTerrain(Terrains.BUILDING)) {
                        osupers = tileManager.supersFor(sHex);
                        bldgHeight = sHex.terrainLevel(Terrains.BLDG_ELEV);
                    } else {
                        osupers = tileManager.orthoFor(sHex);
                        bldgHeight = sHex.terrainLevel(Terrains.BRIDGE_ELEV);
                    }
                    if (!osupers.isEmpty()) {
                        int hash = osupers.get(osupers.size()-1).hashCode();
                        
                        IHex shadowingHex = board.getHex(shadowing);
                        int levelDiff = bldgHeight + shadowingHex.getLevel() - hex.getLevel();
                        Point pS = getHexLocationNoIso(shadowing);
                        ...
                        // so that the shadow base image overlaps the current hex
                        double distance = hexVector.distance(0, 0);
                        distance += 1.5*hex_size.width;
                        if (scale*shadowLength*levelDiff < distance) distance = scale*shadowLength*levelDiff;
                        
                        // when shadowing the hex the building is in, put
                        // the start of the base shadow under the building
                        if (shadowing.equals(c)) {
                            distance = 3*hex_size.width;
                        }
                        
                        Point shadowEnd = new Point(
                                hexVector.x+(int)(lightDirection[0]/shadowLength*distance), 
                                hexVector.y+(int)(lightDirection[1]/shadowLength*distance) );

                        // draw the correctly displaced shadow base image
                        gs.drawImage(buildingShadows.get(hash), 
                                shadowEnd.x-terrainShadowEnd.x-(int)(5*scale), 
                                shadowEnd.y-terrainShadowEnd.y-(int)(5*scale), null);
                    }
                }
            ...
            // Transfer the shadow onto the hex image
            Composite svComp = g.getComposite();
            if (game.getPlanetaryConditions().getLight() == PlanetaryConditions.L_DAY) {
    ...
    /**
     * Returns the absolute position of the upper-left hand corner of the hex
     * graphic without regard for isometric displacement
     */
    private Point getHexLocationNoIso(int x, int y) {
        int ypos = (y * (int) (HEX_H * scale))
                   + ((x & 1) == 1 ? (int) ((HEX_H / 2) * scale) : 0);
        return new Point(x * (int) (HEX_WC * scale), ypos);
    }
    
    private Point getHexLocationNoIso(Coords c) {
        return getHexLocationNoIso(c.getX(), c.getY());
    }
