...
public class WeaponHandler implements AttackHandler, Serializable {
    ...
    protected boolean handleSpecialMiss(Entity entityTarget, boolean bldgDamagedOnMiss,
        ...
        if ((entityTarget != null)
            ...
            gameManager.tryIgniteHex(target.getBoardLocation(), subjectId, false, false,
    ...
    protected void handleEntityDamage(Entity entityTarget,
        ...
        Hex targetHex = game.getBoard(ae).getHex(target.getPosition());
    ...
    protected void handleClearDamage(Vector<Report> vPhaseReport, Building bldg, int nDamage,
        ...
        Vector<Report> clearReports = gameManager.tryClearHex(target.getBoardLocation(), nDamage, subjectId);
    ...
    public int checkTerrain(int nDamage, Entity entityTarget, Vector<Report> vPhaseReport) {
        ...
        Hex hex = game.getBoard(ae).getHex(entityTarget.getPosition());
        ...
        if (game.getOptions().booleanOption(OptionsConstants.ADVCOMBAT_TACOPS_WOODS_COVER)
            ...
            gameManager.tryClearHex(entityTarget.getBoardLocation(), treeAbsorbs, ae.getId());
            Report.addNewline(vPhaseReport);
    ...
    public int checkLI(int nDamage, Entity entityTarget, Vector<Report> vPhaseReport) {
        ...
        for (Coords curr : coords) {
            ...
            if (!game.getBoard(ae).contains(curr)) {
                continue;
            }
            Terrain smokeHex = game.getBoard(ae).getHex(curr).getTerrain(Terrains.SMOKE);
            if (game.getBoard(ae).getHex(curr).containsTerrain(Terrains.SMOKE)
                    && ((smokeHex.getLevel() == SmokeCloud.SMOKE_LI_LIGHT)
                    || (smokeHex.getLevel() == SmokeCloud.SMOKE_LI_HEAVY))) {

                int levit = ((game.getBoard(ae).getHex(curr).getLevel()) + 2);
