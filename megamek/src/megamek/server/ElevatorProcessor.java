...
public class ElevatorProcessor extends DynamicTerrainProcessor {
    ...
    void doEndPhaseChanges(Vector<Report> vPhaseReport) {
        ...
        for (Iterator<BoardLocation> i = elevators[roll].positions.iterator(); i.hasNext();) {
            BoardLocation boardLocation = i.next();
            Coords c = boardLocation.getCoords();
            Hex hex = gameManager.getGame().getHex(boardLocation);
        ...
        for (Board board : gameManager.getGame().getBoards()) {
            ...
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    if (board.getHex(x, y).containsTerrain(Terrains.ELEVATOR)) {
                        exits = board.getHex(x, y).getTerrain(Terrains.ELEVATOR)
                                .getExits();
                        ...
                        for (int z = 0; z < 6; z++) {
                            if ((exits & 1) == 1) {
                                elevators[z].positions.add(new BoardLocation(new Coords(x, y), board.getBoardId()));
                            }
                            exits >>= 1;
                        }
                    }
