...
public class ServerHelper {
    ...
    public static boolean infantryInOpen(Entity te, Hex te_hex, Game game, 
        ...
        if (isPlatoon && !te.isDestroyed() && !te.isDoomed() && !ignoreInfantryDoubleDamage
            ...
            if (te_hex == null) {
                te_hex = game.getBoard(te).getHex(te.getPosition());
            }
    ...
    public static void sinkToBottom(Entity entity) {
        if ((entity == null) || !entity.getGame().hasBoardLocation(entity.getBoardLocation())) {
            return;
        }
        
        Hex fallHex = entity.getBoard().getHex(entity.getPosition());
        int waterDepth = 0;
    ...
    public static boolean detectHiddenUnits(Game game, Entity detector, Coords detectorCoords,
        ...
        for (Entity detected : hiddenUnits) {            
            ...
            LosEffects los = LosEffects.calculateLOS(game, detector, detected, detectorCoords, detected.getPosition(), false, null);
